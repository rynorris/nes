...
impl Compositor {
    pub fn new(
        ...
        let mut main_window = video
            .window("NES", 256 * SCALE as u32, 240 * SCALE as u32)
            .position_centered()
            .opengl()
            .build()
            .unwrap();
        ...
        let canvas = main_window.into_canvas().accelerated().build().unwrap();

        let texture_creator = canvas.texture_creator();
        let nes_texture = match texture_creator.create_texture_static(
            Some(pixels::PixelFormatEnum::RGB24),
            256,
            240,
        ) {
        ...
        let debug_window = video
            .window("NES (Debug)", 256 * 2 as u32, 472 * 2 as u32)
            .opengl()
            .hidden()
            .build()
            .unwrap();

        let mut debug_canvas = debug_window.into_canvas().accelerated().build().unwrap();

        debug_canvas.set_scale(2.0, 2.0).unwrap();

        let debug_texture_creator = debug_canvas.texture_creator();

        let pattern_texture = match debug_texture_creator.create_texture_static(
            Some(pixels::PixelFormatEnum::RGB24),
            256,
            128,
        ) {
        ...
        let nametable_texture = match debug_texture_creator.create_texture_static(
            Some(pixels::PixelFormatEnum::RGB24),
            512,
            480,
        ) {
        ...
        let sprite_texture = match debug_texture_creator.create_texture_static(
            Some(pixels::PixelFormatEnum::RGB24),
            256,
            32,
        ) {
        ...
        let palette_texture = match debug_texture_creator.create_texture_static(
            Some(pixels::PixelFormatEnum::RGB24),
            256,
            32,
        ) {
        ...
        let waveform_texture = match debug_texture_creator.create_texture_static(
            Some(pixels::PixelFormatEnum::RGB24),
            256,
            160,
        ) {
    ...
    fn render_ppu_debug(&mut self) {
        ...
        self.ppu_debug.consume(|buffers| {
            pattern_texture
                .update(None, &buffers.patterns, PPUDebug::PATTERN_WIDTH * 3)
                .unwrap();
            nametable_texture
                .update(None, &buffers.nametables, PPUDebug::NAMETABLE_WIDTH * 3)
                .unwrap();
            sprite_texture
                .update(None, &buffers.sprites, PPUDebug::SPRITE_WIDTH * 3)
                .unwrap();
            palette_texture
                .update(None, &buffers.palettes, PPUDebug::PALETTE_WIDTH * 3)
                .unwrap();
        });

        let _ = self
            .debug_canvas
            .copy(&pattern_texture, None, rect::Rect::new(0, 0, 256, 128));
        let _ = self
            .debug_canvas
            .copy(&nametable_texture, None, rect::Rect::new(0, 136, 256, 256));
        let _ = self
            .debug_canvas
            .copy(&sprite_texture, None, rect::Rect::new(0, 400, 256, 32));
        let _ = self
            .debug_canvas
            .copy(&palette_texture, None, rect::Rect::new(0, 440, 256, 32));
    ...
    fn render_apu_debug(&mut self) {
        ...
        self.apu_debug.consume(|waveforms| {
            waveform_texture
                .update(None, waveforms, APUDebug::WAVEFORM_WIDTH * 3)
                .unwrap()
        });

        let _ = self
            .debug_canvas
            .copy(&waveform_texture, None, rect::Rect::new(0, 0, 256, 160));
        self.debug_canvas.present();
    }
}
